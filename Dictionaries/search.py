...
def keyword_to_titles(metadata):
    if len(metadata) == 0 or len(metadata[0]) == 0:
        return {}
    keyword_dict = {}
...
def title_to_info(metadata):
    ...
    title_to_info_dict = {}
    for article in metadata:
        title_to_info_dict[article[0]] = {'author' : article[1], 'timestamp' : article[2], 'length' : article[3]}
...
def search(keyword, keyword_to_titles):
        if len(keyword) == 0 or len(keyword_to_titles) == 0:
            return []   
    ...
    for article in article_titles:
        if article in title_to_info:
            if title_to_info[article]['length'] <= max_length:
                article_in_range.append(article)

    return article_in_range
...
def key_by_author(article_titles, title_to_info):
    if len(title_to_info) == 0:
        return {}
    author_articles_dict = {}
    for article in article_titles:
        if title_to_info[article]['author'] in author_articles_dict:
            author_articles_dict[title_to_info[article]['author']].append(article)
        else:
            author_articles_dict[title_to_info[article]['author']] = [article]
...
def filter_to_author(author, article_titles, title_to_info):
    if len(title_to_info) == 0:
        return []
    articles_by_author_only = []
    for article in article_titles:
        if title_to_info[article]['author'] == author:
            articles_by_author_only.append(article)

    return articles_by_author_only
...
def filter_out(keyword, article_titles, keyword_to_titles):
    article_without_keyword = []
    for article in article_titles:
        if keyword not in keyword_to_titles:
            return article_titles
        if article not in keyword_to_titles.get(keyword):
            article_without_keyword.append(article)

    return article_without_keyword
...
def articles_from_year(year, article_titles, title_to_info):
    article_published_in_year = []
    start_date = datetime.date(year, 1, 1)
    end_date = datetime.date(year, 12, 31)
    start_unix_timestamp = time.mktime(start_date.timetuple())
    end_unix_timestamp = time.mktime(end_date.timetuple())
    for article in article_titles:
        if start_unix_timestamp <= title_to_info[article]['timestamp'] <= end_unix_timestamp:
            article_published_in_year.append(article)

    return article_published_in_year
